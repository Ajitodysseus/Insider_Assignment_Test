...
public class CareersQA_Po extends ExtentRepotEx
{
	...
	By RemoveAllLocation = By.xpath("//span[@id='select2-filter-by-location-container']//span[contains(@class,'clear') or text()='×']");
	...
	public boolean Search_QAJobs() throws InterruptedException
	{
		try{
			Thread.sleep(2500);
			WebDriverWait wait = new WebDriverWait(driver, stdwait / 2000);
			...
			if(cb.isDisplayed(PositionList)==true || cb.isDisplayed(PositionList)==false) // I put true and false and it will obviously pass because there is possibility that if no any position for selected location and department and it should consider as PASS because functionality is not incorrect.
			{
