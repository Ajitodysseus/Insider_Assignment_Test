...
public class ImagecaptureConfig extends ExtentRepotEx {
	...
	public static String imagepath = properties.getProperty("reportPath") + "\\Report_" + ExtentRepotEx.timestamp
			+ "\\Screenshot" + ExtentRepotEx.timestamp + "." + format;
	...
	public void Selenium_takeSnapShot(WebDriver driver) throws Exception {
		...
		DestFile = new File(properties.getProperty("reportPath") + "\\Report_" + ExtentRepotEx.timestamp
	...
	public void selenium_reportsnapshotPass(String detail) {
		try {
			...
			cf.pass(detail + test.addScreenCapture(DestFile.getAbsolutePath()
					.replace(properties.getProperty("reportPath") + "\\Report_" + ExtentRepotEx.timestamp + "\\", "")));
			Thread.sleep(2000);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void selenium_reportsnapshotFail(String detail) {
		try {
			CommonFunctions cf = new CommonFunctions();
			cf.fail(detail + test.addScreenCapture(DestFile.getAbsolutePath()
					.replace(properties.getProperty("reportPath") + "\\Report_" + ExtentRepotEx.timestamp + "\\", "")));
			Thread.sleep(2000);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void capturesnapshot_Fail() {
		try {
			 ...
			 * captureRect = new Rectangle(0, 0, screenSize.width / 2, screenSize.height /
			 * 2); BufferedImage screenFullImage = robot.createScreenCapture(captureRect);
	...
	public void capturesnapshot() {
		try {
			...
			output = new File(properties.getProperty("reportPath") + "\\Report_" + ExtentRepotEx.timestamp
					+ "\\Screenshot_" + System.currentTimeMillis() + "." + "jpg");
	...
	public void reportsnapshotPass(String detail) {
		try {
			CommonFunctions cf = new CommonFunctions();
			cf.pass(detail + test.addScreenCapture(output.getAbsolutePath()
					.replace(properties.getProperty("reportPath") + "\\Report_" + ExtentRepotEx.timestamp + "\\", "")));
			Thread.sleep(2000);
		} catch (Exception e) {
			e.printStackTrace();
