...
import PO.InsiderDATA.CareersQA_Po;
import PO.InsiderDATA.CompanyMenu_Po;
import PO.InsiderDATA.OpenInsiderApp_Po;
...
public class InsiderApp_Test extends BaseClass {
	...
	public void Validate_All_QAJobs_Contains() throws InterruptedException {
		...
		try {
			...
			if (properties.getProperty("CareersQAJobs").equalsIgnoreCase("True")) {
				...
				cf.Info("Check that all jobs’ Position contains Quality Assurance, Department contains Quality Assurance, and Location contains Istanbul, Turkey.");
	...
	@Test(priority = 5)
	public void Validate_ViewRole() throws InterruptedException {
		...
		try {
			...
			if (properties.getProperty("CareersQAJobs").equalsIgnoreCase("True")) {
				...
				cf.Info("Click the View Role button and check that this action redirects us to the Lever Application form page.");

				TestStepStatus = cq.Validate_View_Role();
				Thread.sleep(2000);
